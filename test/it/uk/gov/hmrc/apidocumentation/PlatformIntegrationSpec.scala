...
import uk.gov.hmrc.apidocumentation.controllers.ApiPlatformController
...
trait PlatformIntegrationSpec extends UnitSpec with MockitoSugar with ScalaFutures with BeforeAndAfterEach with GuiceOneAppPerTest {
  ...
  trait Setup  {
    val documentationController = new ApiPlatformController(LazyHttpErrorHandler, app.injector.instanceOf[ServiceConfiguration]) {}
