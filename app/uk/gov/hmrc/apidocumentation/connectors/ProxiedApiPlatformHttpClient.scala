...
class ProxiedApiPlatformHttpClient @Inject()(config: ServiceConfiguration, override val auditConnector: AuditConnector, override val wsClient: WSClient)
  ...
  lazy val authorization = Authorization(s"Bearer ${config.getString(s"${config.env}.api-platform.bearer-token")}")
